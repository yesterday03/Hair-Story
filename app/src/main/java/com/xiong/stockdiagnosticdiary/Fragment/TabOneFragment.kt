...
import com.xiong.stockdiagnosticdiary.databinding.FragmentTabOneBinding
...
class TabOneFragment : Fragment() {
    ...
    private fun init(){
        val apps= arrayListOf("项目","发型师")
        binding.vp2.adapter= object : FragmentStateAdapter(requireActivity()) {
            override fun getItemCount(): Int {
               return apps.size
            }

            override fun createFragment(position: Int): Fragment {
                return BlankFragment.newInstance(apps[position])
            }

        }
        TabLayoutMediator(binding.tabLayout1, binding.vp2) { tab, position ->
            tab.text = apps[position]  // 设置每个选项卡的标题
        }.attach()
    }
    companion object {
        /**
